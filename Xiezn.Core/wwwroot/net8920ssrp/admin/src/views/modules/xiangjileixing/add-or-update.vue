<template>
	<div class="addEdit-block" :style='{"padding":"0 30px","margin":"0 auto","color":"#666","borderRadius":"4px","background":"none","width":"96%","fontSize":"16px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' class="input" v-if="type!='info'"  label="相机类型" prop="xiangjileixing">
					<el-input v-model="ruleForm.xiangjileixing" placeholder="相机类型" clearable  :readonly="ro.xiangjileixing"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 30px 0","fontSize":"inherit","color":"inherit"}' v-else class="input" label="相机类型" prop="xiangjileixing">
					<el-input v-model="ruleForm.xiangjileixing" placeholder="相机类型" readonly></el-input>
				</el-form-item>
			</template>
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `xiangjileixing/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.xiangjileixingCrossAddOrUpdateFlag = false;
											this.parent.search();
